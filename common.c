/* $Id: common.c,v 1.63 2004/10/30 12:46:28 wojtekka Exp $ */
 ...
 */
/* RL (prostsza obsluga debugowania) */
void gg_debug_defproc(int level, const char *format, ...)
{
	va_list ap;
...
gg_debug_ftype gg_debug = gg_debug_defproc;
...
char *gg_read_line(int sock, char *buf, int length)
{
	...
	for (; length > 1; buf++, length--) {
		do {
			if ((ret = read(sock, buf, 1)) == -1 && s_errno != EINTR) {
...
/* RL - pozostajê przy mojej oryginalnej wersji */

struct gg_thread * gg_threads = 0;
//char gg_thread_socket_inuse = 0;
...
gg_thread* gg_thread_find(int thread_id, int socket, gg_thread *** p_wsk) {
    gg_thread * wsk = gg_threads;     // Aktualny element
    while (wsk && gg_threads) {
...
{
   	char close = thread_id==-1||socket==-1; // Cokolwiek bedzie zamykane.
    gg_thread * wsk = gg_threads;     // Aktualny element
    gg_thread ** p_wsk = &gg_threads; // Wskaznik do ustawianego elementu.
	gg_thread_socket_lock();
	if (!thread_id) thread_id = GetCurrentThreadId();
	wsk = gg_thread_find(thread_id, socket, &p_wsk);
	if (wsk) {
		if (close) {
            closesocket(wsk->socket);
			WSACloseEvent(wsk->event);
         	*p_wsk=wsk->next;
			...
			socket = wsk->socket; 
			goto finish; // Socket zostaje zwrocony
        } else {wsk->socket=socket; goto finish;} // Socket zostaje ustawiony
	}
	// nic nie znalaz³em i zamykam...
    if (close && socket!=-1) {closesocket(socket);} 
    if (close || !socket) {goto finish;} // Nic nie znalazlem i wychodze
    // Dodaje nowy element
    wsk = malloc(sizeof(gg_thread));
    wsk->id = thread_id;
    wsk->socket = socket;
	wsk->event = WSACreateEvent();
//    WSAEventSelect(socket , wsk->event , FD_READ | FD_CLOSE);
