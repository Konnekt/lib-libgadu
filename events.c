/* $Id: events.c,v 1.100 2005/07/25 19:30:10 porridge Exp $ */
...
struct gg_event *gg_watch_fd(struct gg_session *sess)
{
	...
	switch (sess->state) {
		case GG_STATE_RESOLVING:
		{
			...
			/* HAO | nie jestem pewien, ale to chyba moje :) */
...
*/			
		...
		case GG_STATE_READING_DATA:
		{
			...
			if (strncmp(buf, "HTTP/1.", 7) || strncmp(buf + 9, "200", 3)) {
				...
				/* HAO - przestawia adres serwera*/
				if (gg_proxy_enabled) sess->hub_addr = sess->server_addr;
...
#ifdef __GG_LIBGADU_HAVE_OPENSSL
			/*RL: Tylko gdy proxy jest wylaczone!*/
			if (!sess->proxy_port && sess->ssl) {
...
/*RL: zapêtlenie odczytywania na wypadek gdyby socket by³ w trybie overlapped*/
SSL_retry:
...
//					gg_debug(GG_DEBUG_MISC, "// gg_watch_fd() SSL_connect() wants to write\n");
		...
		{
/* RL - Najpierw proxy, potem czytanie...
			struct gg_header *h;			
			struct gg_welcome *w;
			struct gg_login60 l;
			unsigned int hash;
...
#ifdef __GG_LIBGADU_HAVE_OPENSSL
				/*RL: Odczytalismy proxy, teraz mozemy nawiazac polaczenie po SSL*/
				if (sess->ssl) {
					SSL_set_fd(sess->ssl, sess->fd);
					sess->state = GG_STATE_TLS_NEGOTIATION;
					sess->check = GG_CHECK_WRITE;
