...
/* RL: mo¿e siê zdarzyæ, ¿e która¿ z funkcji z W32 przestawi socket na nie blokujacy.
   W tym wypadku, recv bêdzie rzuca³ b³êdami na lewo i prawo... Wypada³oby siê przed tym
   zabezpieczyæ...*/
#ifdef _WIN32
	do {
#endif
	...
	if (sess->ssl) {
		...
		if (res < 0) {
			err = sess->ssl ? SSL_get_error(sess->ssl, res) : 0;
...
#endif
		/*RL: na W32 read nie dzia³a na socketach nie wiedzieæ czemu...
		  Wszêdzie podmienione jest to przez #define, ale tutaj mo¿emy wstawiæ orygina³ :)
		*/
...
int gg_send_packet(struct gg_session *sess, int type, ...)
{
	...
	if ((unsigned)(res = gg_write(sess, tmp, tmp_length)) < tmp_length) {
        s_errno;
...
struct gg_session *gg_login(/*RL: const*/ struct gg_login_params *p)
{
	...
	sess->port = (p->server_port) ? p->server_port : (p->tls ? GG_HTTPS_PORT : GG_DEFAULT_PORT);
	...
	if (!p->async) {
		struct in_addr a;
		/* HAO | proxy_http_only*/
        if ((!p->server_addr || !p->server_port) || (!gg_proxy_http_only && gg_proxy_enabled)) {
			if ((a.s_addr = inet_addr(hostname)) == INADDR_NONE) {
				struct in_addr *hn;
				...
				if (!(hn = gg_gethostbyname(hostname))) {
					...
					a.s_addr = hn->s_addr;
                    /* HAO | ustawiamy server_addr na pozniej */
   				    if (gg_proxy_username && gg_proxy_http_only) {
						free(hn);
					    if ((hn = gg_gethostbyname(GG_APPMSG_HOST))!=0) {
                        	sess->server_addr = hn->s_addr;
						}
                    }
		...
		/* HAO | Proxy na nie synchronicznych */
        if (gg_proxy_enabled) sess->proxy_addr = sess->hub_addr;
